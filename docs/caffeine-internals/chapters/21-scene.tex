...
\specbox{Implementation Detail: Entity Remapping}{
During deserialization, entity handles in the file may not match the handles assigned by the current \texttt{ECS::World}. The \texttt{SceneSerializer::parsePayload} function maintains an \texttt{unordered_map<u32, ECS::Entity>} to remap old IDs to new ones, ensuring that the \texttt{Parent} components correctly point to the newly created entities in the current session.
}
