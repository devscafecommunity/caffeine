...
\begin{specbox}{Design Rationale: Fixed Buffer Logging}
...
\begin{specbox}{Design Rationale: Fixed Scope Storage}
The profiler uses a pre-allocated array of 256 \texttt{InternalScopeData} entries. While this limits the number of unique scopes that can be tracked simultaneously, it guarantees that entering or exiting a profiled region never triggers a memory allocation. This is vital for profiling tight loops or high-frequency functions where the overhead of a hash map or dynamic vector would skew the results.
\end{specbox}
